...
# Video 2 
# Click play to run the codeblock and the video will display in the markdown document. 
embed_youtube("2LCO4xFLIbU", width = "640", height="480", frameborder = 0,
allowfullscreen = FALSE, query = NULL)
...
# A: Two. Hancock, Georgia and Galax City, Virginia.
...
# A: Galax and Emporia (independent cities) and Northampton County.
...
# A: 14 places had zero and had something afterward, which can't be calculated as a percentage change. But eclusding that it is Luce County, Michigan.
...
exercise1 <-covid_county_monthly_death_rate %>%
...
exercise2 <-covid_county_monthly_case_rate %>%
...
# Question: Which New York county had the highest percentage change in cumulative death rate from July to August?

exercise3 <-covid_county_monthly_death_rate %>%
  select(county, state, d_2020_07_30, d_2020_08_30) %>%
  filter (state =="New York") %>%
  mutate(july_aug_pct_change = (d_2020_08_30 - d_2020_07_30)/d_2020_07_30) %>%
  arrange(desc(july_aug_pct_change))

exercise3
#Answer:  Herkimer County with 4x as many deaths.
