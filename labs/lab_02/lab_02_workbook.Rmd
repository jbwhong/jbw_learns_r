...
```{r}
arcos_md_blatimorecity <- read_csv("data/arcos-md-baltimore-city-24510-pharmacy.csv")
```
...
```{r}
glimpse(arcos_md_blatimorecity)
```
...
```{r}
arcos_md_baltimorecity_pharmacies <- arcos_md_blatimorecity %>%
  select(buyer_name)
...
```{r}
arcos_wv_baltimorecity_trimmed <- arcos_md_blatimorecity %>%
  select (buyer_name, buyer_dea_no, buyer_city, total_dosage_unit, total_records)
```
...
By default, the arrange function sorts the column name you feed it from lowest to highest value. 
...
```{r}
arcos_md_baltimorecity_sorted <- arcos_wv_baltimorecity_trimmed %>%
  arrange(desc(total_dosage_unit))
...
```{r}
# Answer to question 1: Giant 166
answer_task_1 <- arcos_md_blatimorecity %>%
...
```{r}
# Answer to question 2: OSHINSKY, ALAN E MD and JAKOBOVITS, JULIAN MD
4

answer_task_2 <- arcos_md_blatimorecity %>%
