...
lowest_counties_per_state <- covid_county %>%
  group_by(state) %>%
  summarise(total_counties = n()) %>%
...
lowest_counties_per_state

# Q: which state has the smallest number of counties? 
# A: Delaware. (DC is not a state yet.)
...
#Q: What is the average number of deaths in Maryland counties_per_state

average_deaths_md_counties <- covid_county %>%
    filter (state =="Maryland") %>%
summarise(deaths_in_average_county = mean(deaths))

#display it

average_deaths_md_counties
